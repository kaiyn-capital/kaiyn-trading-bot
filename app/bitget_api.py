...
class BitgetAPIClient:
    ...
    async def set_position_mode(self, product_type: str = 'USDT-FUTURES', pos_mode: str = 'hedge_mode') -> Dict:
        """設置持倉模式 - 預設雙向持倉以支援多空策略"""
        data = {
            'productType': product_type,
            'posMode': pos_mode  # hedge_mode 雙向持倉(可同時持有多空), one_way_mode 單向持倉
        }
...
class BitgetTradeManager:
    ...
    async def place_market_order(self, user_id: int, encrypted_credentials: Tuple[str, str, str],
                               symbol: str, side: str, size: str, client_order_id: str = None,
                               margin_coin: str = 'USDT', trade_side: str = 'open', 
                               stop_loss_price: float = None, take_profit_price: float = None) -> Dict:
        """下合約市價單 - 添加必要的tradeSide參數"""
        client = self._get_client(user_id, encrypted_credentials)
    ...
    async def get_order_status(self, user_id: int, encrypted_credentials: Tuple[str, str, str],
                             symbol: str, order_id: str = None, client_order_id: str = None) -> Dict:
        """查詢訂單狀態"""
        client = self._get_client(user_id, encrypted_credentials)
        return await client.get_order_info(symbol, order_id, client_order_id)
    
    async def cancel_order(self, user_id: int, encrypted_credentials: Tuple[str, str, str],
                          symbol: str, order_id: str = None, client_order_id: str = None) -> Dict:
        """取消訂單"""
        client = self._get_client(user_id, encrypted_credentials)
