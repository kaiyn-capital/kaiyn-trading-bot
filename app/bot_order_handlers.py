...
class OrderHandlersMixin:
    ...
    async def _handle_place_order_callback(self, query, user, data):
        ...
        try:
            await self._send_private_message(query, user, "🔄 正在获取当前市价与交易规则...")
            ...
            try:
                ...
                if "entry price" in str(e):
                    ...
                    await self._send_private_message(query, user, "❌ 进场价格错误，无法计算仓位")
                    return
                await emit_audit_event(
                    self,
                    user,
                ...
                await self._send_private_message(query, user, "❌ 止损价格设置错误，无法计算仓位")
                return
            ...
            if not validation.is_valid:
                ...
                await self._send_private_message(query, user, validation.error_message or "❌ 无法下单")
                return
    ...
    async def _handle_confirm_pending_order_callback(self, query, user, data):
        ...
        try:
            ...
            if not pending_order:
                ...
                await self._send_private_message(query, user, "❌ 找不到这笔待确认订单，请重新点击最新信号下单。")
                return
            ...
            if status == "expired":
                ...
                await self._send_private_message(query, user, "❌ 这笔待确认订单已过期，请重新点击信号下单。")
                return
            ...
            if status != "processing":
                ...
                await self._send_private_message(query, user, f"⚠️ 这笔待确认订单目前状态为 {status}，无法重复执行。")
                return
    ...
    async def _handle_cancel_pending_order_callback(self, query, user, data):
        ...
        if status == "cancelled":
            ...
            await self._send_private_message(query, user, f"⚠️ 这笔待确认订单目前状态为 {status}，无法取消。")
