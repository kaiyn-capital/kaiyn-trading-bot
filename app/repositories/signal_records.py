...
class SignalRecordRepository:
    ...
    async def create_signal_record(
        ...
        *,
        ...
        async with self.db.get_session() as session:
            ...
            session.add(record)
            await session.flush()
            return signal_record_to_dict(record)
    ...
    async def get_by_public_id(self, public_id: str) -> dict | None:
        async with self.db.get_session() as session:
            ...
            return signal_record_to_dict(record) if record else None
    ...
    async def add_channel_message(
        ...
        *,
        ...
        async with self.db.get_session() as session:
            ...
            session.add(message)
            await session.flush()
            return signal_channel_message_to_dict(message)

    async def get_channel_messages(self, signal_record_id: int) -> list[dict]:
        async with self.db.get_session() as session:
            ...
            return [signal_channel_message_to_dict(message) for message in result.scalars().all()]


def signal_record_to_dict(record: SignalRecord) -> dict:
    return {
        "id": record.id,
...
def signal_channel_message_to_dict(message: SignalChannelMessage) -> dict:
    return {
        "id": message.id,
