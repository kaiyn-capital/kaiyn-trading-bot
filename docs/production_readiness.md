...
第六優先。此項已完成基礎版，正式運營時可從 DB/log 追蹤主要操作。

已完成：

- 使用現有 `system_logs` 作為審計儲存，不新增 DB schema。
- 審計事件固定 `module="audit"`，並以 `function` 記錄操作類型。
- 記錄管理員操作：新增發單員、頻道新增/重新啟用/刪除、topic 設定/清除、廣播、指定頻道發送。
- 記錄發單員操作：發送信號內容摘要、目標頻道數量、成功與失敗數量。
- 記錄使用者下單操作：點擊市價/掛單、建立 pending order、確認、取消、過期、重複確認、下單成功與失敗原因。
- 新增 `/admin_audit [limit]`，預設 10 筆、最多 30 筆，顯示 UTC+8 的近期操作摘要。

本輪刻意不做：

- 不建立獨立不可竄改 audit table。
- 不新增更細的角色或權限分級。
- 不保存 API key、secret、passphrase、完整 Bitget response、完整下單 payload。
- 廣播與手動發送只保存訊息長度與短預覽，不保存完整文字。
