...
- db-backup 備份失敗通知管理員。
- 可新增簡單 `/health` 或 `/admin_health` 指令查看 DB、Bot、備份狀態。
...
- 新增一段備份還原 runbook。
- 提供將 `backups/*.sql.gz` 還原到臨時 PostgreSQL DB 的指令。
- 記錄建議頻率：正式部署前至少測一次，之後每月或重大改版後測一次。
- 確認還原後 Alembic version、主要資料表、近期資料筆數正常。
