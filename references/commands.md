...
```text
/send_signal 交易對 方向 entry[進場價1 進場價2] sl[止損價] tp[止盈價1 止盈價2] [備註文字]
```
...
/send_signal ETHUSDT short entry[3200 3250] sl[3300] tp[3100 3000 2900] 等待回踩後執行
```

格式規則：

- `方向` 僅支援 `long` 或 `short`。
- `entry[]` 必填，可填 1 或 2 個進場價。
- `sl[]` 必填，必須填 1 個止損價。
- `tp[]` 必填，可填 1 個以上止盈價。
...
- 信號底部時間戳使用 UTC+8。
- `/send_signal` 會先在私人聊天回傳預覽與「确认转发 / 取消」按鈕；預覽 TTL 預設 5 分鐘，可由 `USER_SESSION_TTL_SECONDS` 調整，session 會加密保存於 PostgreSQL。預覽未確認、被新預覽覆蓋或取消時，不會轉發到群組。
...
- Bot 會先在私人聊天回傳更新圖表預覽與「确认转发 / 取消」按鈕；預覽 TTL 沿用一般 session TTL，並加密保存於 PostgreSQL。
